...
$(document).on(touchEvent,'.mapTileContainer.potentialPlacement',function(){
	...
	setTimeout(function(){
		...
		rotateTileAllowed = true;
		 // generate PNG of the map
		 html2canvas(document.querySelector("#mapContainer"), { useCORS: true }).then(canvas => {
			// get the data URL of the canvas
			var imgData = canvas.toDataURL("image/png");
		
			// send the data URL to a server-side script
			$.post("save_image.php", { data: imgData }, function(response) {
